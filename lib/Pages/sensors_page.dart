...
class _SensorsPageState extends State<SensorsPage> {
  ...
  String classifyRegion(double lat, double lon) {
    if (lat >= 30 && lat <= 40 && lon >= -100 && lon <= -85)
      return 'Central USA';
  ...
  Future<void> getLocationAndClassifyRegion() async {
    ...
    if (permission == LocationPermission.deniedForever) {
      ScaffoldMessenger.of(context).showSnackBar(
         SnackBar(
          content: Text("Location permissions are permanently denied".tr()),
        ),
      );
      return;
    }
    ...
    print(
      "Location: (${position.latitude}, ${position.longitude}) => Region: $region",
    );
  }
  ...
  void randomizeValues() {
    setState(() {
      soilMoistureController.text = (random.nextDouble() * 100).toStringAsFixed(
        1,
      );
      soilPHController.text = (4 + random.nextDouble() * 4).toStringAsFixed(2);
      temperatureController.text = (15 + random.nextDouble() * 20)
          .toStringAsFixed(1);
      ...
      totalDaysController.text = (30 + random.nextDouble() * 120)
          .toStringAsFixed(1);
      yieldController.text = (1000 + random.nextDouble() * 5000)
          .toStringAsFixed(1);
      latitudeController.text = (-90 + random.nextDouble() * 180)
          .toStringAsFixed(5);
      longitudeController.text = (-180 + random.nextDouble() * 360)
          .toStringAsFixed(5);
  ...
  void submit() async {
    final url = Uri.parse(
      "https://missingbreath-growth.hf.space/GeoSensor",
    ); // Replace this
                  ...
                  children: [
                    ...
                    ...List.generate(categories.length, (i) {
                      return Column(
                        ...
                        children: [
                          Text(
                            "${categories[i]}: ${(percentages[i] * 100).toStringAsFixed(1)}%",
    ...
    return Stack(
      children: [
        ...
        Positioned(
          bottom: 0,
          right: 0,
          child: Image.asset('assets/images/leaf_bottom_right.png', width: 130),
        ),
            ...
            children: [
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    Text(
                      'Sensors & GeoLocation Based Detection'.tr(),
                      style: const TextStyle(
                        fontSize: 15,
